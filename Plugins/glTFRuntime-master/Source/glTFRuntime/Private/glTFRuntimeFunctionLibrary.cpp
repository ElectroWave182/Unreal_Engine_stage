...
FglTFRuntimeMeshLOD UglTFRuntimeFunctionLibrary::glTFMergeRuntimeLODsWithTransforms(
	const TArray<FglTFRuntimeMeshLOD>& RuntimeLODs,
	const TArray<FTransform>& Transforms)
{
	FglTFRuntimeMeshLOD NewRuntimeLOD;

	for (int32 LODIndex = 0; LODIndex < RuntimeLODs.Num(); LODIndex++)
	{
		// Copie du LOD pour pouvoir modifier ses vertices
		FglTFRuntimeMeshLOD CurrentLOD = RuntimeLODs[LODIndex];

		// Applique la transform si elle est fournie pour cet index
		const bool bHasTransform = Transforms.IsValidIndex(LODIndex)
			&& !Transforms[LODIndex].Equals(FTransform::Identity);

		if (bHasTransform)
		{
			const FTransform& NodeTransform = Transforms[LODIndex];

			for (FglTFRuntimePrimitive& Primitive : CurrentLOD.Primitives)
			{
				// -- Positions --
				for (FVector& Position : Primitive.Positions)
				{
					Position = NodeTransform.TransformPosition(Position);
				}

				// -- Normales : rotation uniquement, pas de translation/scale --
				for (FVector& Normal : Primitive.Normals)
				{
					Normal = NodeTransform.TransformVectorNoScale(Normal);
					Normal.Normalize();
				}

				// -- Tangentes : idem, rotation uniquement --
				for (FVector4& Tangent : Primitive.Tangents)
				{
					FVector T(Tangent.X, Tangent.Y, Tangent.Z);
					T = NodeTransform.TransformVectorNoScale(T);
					T.Normalize();
					Tangent = FVector4(T.X, T.Y, T.Z, Tangent.W); // W = handedness, inchangé
				}
			}
		}

		// Merge dans le LOD final (logique originale inchangée)
		NewRuntimeLOD.Primitives.Append(CurrentLOD.Primitives);
		NewRuntimeLOD.AdditionalTransforms.Append(CurrentLOD.AdditionalTransforms);

		if (NewRuntimeLOD.Skeleton.Num() == 0)
		{
			NewRuntimeLOD.Skeleton = CurrentLOD.Skeleton;
		}
		if (!NewRuntimeLOD.bHasNormals)
		{
			NewRuntimeLOD.bHasNormals = CurrentLOD.bHasNormals;
		}
		if (NewRuntimeLOD.bHasTangents)
		{
			NewRuntimeLOD.bHasTangents = CurrentLOD.bHasTangents;
		}
		if (!NewRuntimeLOD.bHasUV)
		{
			NewRuntimeLOD.bHasUV = CurrentLOD.bHasUV;
		}
		if (!NewRuntimeLOD.bHasVertexColors)
		{
			NewRuntimeLOD.bHasVertexColors = CurrentLOD.bHasVertexColors;
		}
	}
